...
//        r.javaRDD().repartition(1).saveAsTextFile(output + "warmup");
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        ...
        // standard sql query
        d.createOrReplaceTempView("flights");
        Dataset<Row> rA = spark.sql("SELECT DISTINCT destcityname FROM flights WHERE origincityname = 'Seattle, WA' ORDER BY destcityname");
        rA.show();
        return rA.javaRDD();
    }
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        JavaRDD<Row> rd = d.javaRDD();

        // filter non_cancelled flights
        rd = rd.filter(t -> t.getInt(CANCELLED) == 0);

        //create pair mapping
        JavaPairRDD<Tuple2<String, Integer>, Integer> city_month_count =
                rd.mapToPair(t -> new Tuple2<Tuple2<String, Integer>, Integer>
                        (new Tuple2<String, Integer>(t.getString(ORIGIN_CITY_NAME), t.getInt(MONTH)), 1));

        // reducebykey
        JavaPairRDD<Tuple2<String, Integer>, Integer> r3 =
                city_month_count.reduceByKey((v1, v2) -> v1 + v2);

        //extract values and convert back to javaRDD
        JavaRDD<Row> result =
                r3.map(t -> RowFactory.create(t._1()._1(), t._1()._2(), t._2()));
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        ...
        //filter non_delay flights
        rd = rd.filter(t -> t.get(DEP_DELAY) != null);

        //aggregate delay time for city
        JavaPairRDD<String, Tuple2<Integer, Integer>> city_delay_count =
                rd.mapToPair(t -> new Tuple2<String, Tuple2<Integer, Integer>>(t.getString(ORIGIN_CITY_NAME),
                        new Tuple2<Integer, Integer>(t.getInt(DEP_DELAY), 1)));
        //reducebykey to aggregate
        city_delay_count = city_delay_count.reduceByKey((d1, d2) -> new Tuple2<Integer, Integer>(d1._1() + d2._1(), d1._2() + d2._2()));

        //caculate avg delay
        JavaPairRDD<String, Double> avg_delay =
                city_delay_count.mapToPair(t -> new Tuple2<String, Double>(t._1(), (t._2()._1() / t._2()._2()) * 1.0));

        //map to javardd
        JavaRDD<Row> result = avg_delay.map(t -> RowFactory.create(t._1(), t._2()));
        result.foreach(string -> System.out.println(string));
        return result;
    }
